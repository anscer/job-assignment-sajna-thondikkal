...
const createUser = asyncHandler(async(req,res,next)=>{
    const {user_name,password,user_role,phone} = req.body;
    const hashedPassword = hashPassword(password);
    const newUser = await userRepositories.createUser({user_name,password:hashedPassword,user_role,phone});
    if(newUser){
...
const updateUser = asyncHandler(async(req,res,next)=>{
    ...
    const {user_name,password,user_role,phone} = req.body;
    const hashedPassword = hashPassword(password);
    const updated = await userRepositories.updateUser(id,{user_name,password:hashedPassword,user_role,phone});
    if(updated){
